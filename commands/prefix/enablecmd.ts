...
const commandExport: CommandPrefix = {
	...
	async execute(message, client) {
		...
		const texts = client.translate(message.guild?.preferredLocale as string, "prefix/enablecmd");
		...
		commands.splice(commands?.indexOf(commands.find(cmd => cmd === command) as string), 1);
		client.database.set(`/${message.guild?.id}/disabled`, commands);
