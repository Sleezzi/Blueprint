...
const event: Event = {
	name: "Reaction Add",
	event: "messageReactionRemove", // Send a log message in the console when the bot shutdown
	type: "on",
	async execute([reaction, user]: [MessageReaction | PartialMessageReaction, User | PartialUser], client) {
		try {
			...
			const texts = client.translate(reaction.message.guild?.preferredLocale as string, `events/reactionRemove`);
