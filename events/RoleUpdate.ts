...
const event: Event = {
	name: "Role edited",
	event: "roleUpdate", // Send a log message in the console when the bot shutdown
	type: "on",
	async execute([role]: [Role], client) {
		try {
			...
			const texts = client.translate(role.guild?.preferredLocale as string, `events/roleUpdate`);
