...
const event: Event = {
	name: "MemberJoin",
	event: "guildMemberAdd", // When a member is added to a server
	type: "on",
	async execute([member]: [GuildMember], client) {
		try {
			const welcome: DatabaseInterface["server"]["modules"]["join"] = await client.database.get(`/${member.guild.id}/modules/join`); // Get the welcome channel id
			if (
				welcome.channelId && // If there is none
				member.guild.channels.cache.find(c => c.id === welcome.channelId)  && // If the channel does not exist/no longer exists
				member.guild.channels.cache.find(c => c.id === welcome.channelId)?.permissionsFor(member.guild.members.cache.find(member => member.id === client.user?.id) as GuildMember)?.has("SendMessages") && // If the bot can't send messages in this channel
				!member.user.bot // If the new member is a bot
			) {
				(member.guild.channels.cache.find(c => c.id === welcome.channelId) as TextChannel).sendTyping();
				...
				ctx.fillText(welcome.message?.replace(/\$user/g, member.user.username.toUpperCase()).replace(/\$server/g, member.guild.name) || `${globalsTexts.welcome}!`, properties.welcome.x || 0, properties.welcome.y || 0); // Writes the member level
				ctx.strokeText(welcome.message?.replace(/\$user/g, member.user.username.toUpperCase()).replace(/\$server/g, member.guild.name) || `${globalsTexts.welcome}!`, properties.welcome.x || 0, properties.welcome.y || 0);
				...
				const attachment = new AttachmentBuilder(image.toBuffer(), {name: `${member.user.tag.toLowerCase()}'s welcome image.png`});
				await (member.guild.channels.cache.find(c => c.id === welcome.channelId) as TextChannel).send({ files: [attachment] });
			}
			const role: DatabaseInterface["server"]["modules"]["autorole"] = await client.database.get(`${member.guild.id}/modules/autorole`);
			if (
				typeof role === "string" &&
				member.guild.roles.cache.find(r => r.id === role) &&
				member.guild.roles.cache.find(r => r.id === role)?.editable &&
				member.manageable
			) {
